...
export class ContaPoupanca extends Conta {
    ...
    simularRendimento(meses, juro, valorAdicional) {
        ...
        valorAdicional
            ? (rendimentoTotal = valorAdicional + this._saldo)
            : (rendimentoTotal = this._saldo);

        rendimentoTotal += rendimentoTotal * Math.pow(1 + juro / 100, meses);

        !valorAdicional
            ? console.log(
                  `A conta com R$${this._saldo} rendendo ${juro.toFixed(
                      2
                  )}% ao mês irá totalizar R$${(
                      rendimentoTotal - this._saldo
                  ).toFixed(
                      2
                  )} em ${meses} meses, totalizando R$${rendimentoTotal.toFixed(
                      2
                  )}`
              )
            : console.log(
                  `A conta com R$${
                      this._saldo
                  } somando ${valorAdicional} e rendendo ${juro.toFixed(
                      2
                  )}% ao mês irá totalizar R$${(
                      rendimentoTotal - this._saldo
                  ).toFixed(
